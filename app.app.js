...
class Finanzas {
  ...
  bindUI(){
    ...
    // Navegación de subtabs del Plan de Ahorro
    document.addEventListener("click", (e) => {
      if(e.target.classList.contains("savings-tab")) {
        this.showSavingsView(e.target.dataset.savingsTab);
      }
    });
  ...
  showSavingsView(viewName){
    // Cambiar tabs activos
    document.querySelectorAll('.savings-tab').forEach(t => 
      t.classList.toggle('active', t.dataset.savingsTab === viewName)
    );
    
    // Cambiar vistas activas
    document.querySelectorAll('.savings-view').forEach(v => 
  ...
  renderDashboard(ing,gastos,libre){
    ...
    let consejo = "";
    if(tasa < 10){
      consejo = `<div style="background:#fef3c7;border:1px solid #f59e0b;border-radius:8px;padding:12px;margin-top:12px;font-size:14px">
        💡 <strong>Tip:</strong> Visita la pestaña "📚 Plan de Ahorro" para estrategias que te ayuden a mejorar tu capacidad de ahorro.
      </div>`;
    } else if(tasa >= 20){
      consejo = `<div style="background:#d1fae5;border:1px solid #10b981;border-radius:8px;padding:12px;margin-top:12px;font-size:14px">
        🏆 <strong>¡Excelente!</strong> Tienes gran disciplina de ahorro. Explora el "📚 Plan de Ahorro" para maximizar el crecimiento de tu dinero.
      </div>`;
    }
    
    el.innerHTML=`<div class="item"><b style="color:${color}">${fmt(libre)}</b> de balance — Ahorro ${tasa}%</div>${consejo}`;
  }
  ...
  renderPlanAhorro(){
    this.renderConsejosAhorro();
    this.renderLogrosAhorro();
    this.renderResumenProgreso();
    this.renderConsejoDestacado();
  }

  // Aplicar estrategias de ahorro
  aplicarEstrategia(tipo){
    const d = this.mesData;
    const totalIng = d.ingresos.reduce((s,x)=>s+(x.monto||0),0);
    const totalGas = d.gastosFijos.reduce((s,x)=>s+(x.monto||0),0) + 
                     d.tarjetas.reduce((s,x)=>s+(x.cuotaMensual||0)+(x.manejoMensual||0),0) + 
                     d.creditos.reduce((s,x)=>s+(x.cuotaMensual||0),0) + 
                     d.gastosCompras.reduce((s,x)=>s+(x.monto||0),0);

    switch(tipo){
      case '50-30-20':
        const necesidades = totalIng * 0.5;
        const entretenimiento = totalIng * 0.3;
        const ahorroRecomendado = totalIng * 0.2;
        
        let mensaje = `📊 Aplicando regla 50-30-20:\n\n`;
        mensaje += `💰 Ingresos totales: ${fmt(totalIng)}\n`;
        mensaje += `🏠 Necesidades (50%): ${fmt(necesidades)}\n`;
        mensaje += `🎮 Entretenimiento (30%): ${fmt(entretenimiento)}\n`;
        mensaje += `💎 Ahorro recomendado (20%): ${fmt(ahorroRecomendado)}\n\n`;
        
        if(totalGas > necesidades){
          mensaje += `⚠️ Tus gastos básicos (${fmt(totalGas)}) exceden el 50% recomendado. Revisa tus gastos fijos.`;
        } else {
          mensaje += `✅ Tus gastos básicos están dentro del rango. ¡Puedes ahorrar ${fmt(ahorroRecomendado)} este mes!`;
          
          // Crear meta automática si no existe
          const metaExistente = d.ahorros.find(a => a.nombre.includes("50-30-20"));
          if(!metaExistente){
            d.ahorros.push({
              id: this.uid(),
              nombre: "Meta 50-30-20",
              meta: ahorroRecomendado * 12,
              actual: 0,
              fecha: "01"
            });
            this.save();
            mensaje += `\n\n🎯 Se ha creado una nueva meta de ahorro anual.`;
          }
        }
      ...
      case 'automatico':
        const porcentaje = prompt("¿Qué porcentaje de tus ingresos quieres ahorrar automáticamente? (5-30%)", "15");
        if(porcentaje && !isNaN(porcentaje)){
          const pct = Math.max(5, Math.min(30, parseFloat(porcentaje)));
          const montoAuto = totalIng * (pct / 100);
          
          d.ahorros.push({
            id: this.uid(),
            nombre: `Ahorro Automático ${pct}%`,
            meta: montoAuto * 12,
            actual: 0,
            fecha: "01"
          });
          this.save();
          this.renderAll();
          this.toast(`Meta de ahorro automático creada: ${fmt(montoAuto)} mensual`);
        }
        break;

      case 'metas':
        this.openForm("ahorro");
        break;

      case 'emergencia':
        const gastosEsenciales = d.gastosFijos.reduce((s,x)=>s+(x.monto||0),0) + 
                               d.gastosCompras.filter(x=>x.nombre.toLowerCase().includes('mercado') || 
                                                        x.nombre.toLowerCase().includes('comida') ||
                                                        x.nombre.toLowerCase().includes('transporte')).reduce((s,x)=>s+(x.monto||0),0);
        
        const emergencia3 = gastosEsenciales * 3;
        const emergencia6 = gastosEsenciales * 6;
        
        let msg = `🚨 Fondo de Emergencia Recomendado:\n\n`;
        msg += `📊 Gastos esenciales mensuales: ${fmt(gastosEsenciales)}\n\n`;
        msg += `🛡️ Fondo mínimo (3 meses): ${fmt(emergencia3)}\n`;
        msg += `🏆 Fondo ideal (6 meses): ${fmt(emergencia6)}\n\n`;
        
        const crear = confirm(msg + "¿Deseas crear una meta para tu fondo de emergencia?");
        if(crear){
          const metaEmergencia = prompt("Elige tu meta (3 o 6 meses):", "6") === "6" ? emergencia6 : emergencia3;
          d.ahorros.push({
            id: this.uid(),
          ...
          this.save();
          this.renderAll();
          this.toast("Meta de emergencia creada exitosamente");
        }
        break;
    }
  }
  ...
  calcularAhorro(){
    const meta = parseFloat(document.getElementById("metaCalculadora").value) || 0;
    const plazo = parseInt(document.getElementById("plazoCalculadora").value) || 1;
    const actual = parseFloat(document.getElementById("actualCalculadora").value) || 0;
    
    if(meta <= 0 || plazo <= 0){
      this.toast("Por favor ingresa valores válidos");
      return;
    }

    const faltante = Math.max(0, meta - actual);
    const mensual = faltante / plazo;
    const porcentajeCompleto = (actual / meta) * 100;
    const mesesRestantes = Math.ceil(faltante / (mensual || 1));
    ...
    resultados.innerHTML = `
      ...
      <div class="result-item">
        ...
        <span class="result-value">${fmt(meta)}</span>
      </div>
      <div class="result-item">
        <span>Ahorro actual:</span>
        <span class="result-value">${fmt(actual)}</span>
      </div>
      <div class="result-item">
        <span>Faltante:</span>
        <span class="result-value">${fmt(faltante)}</span>
      </div>
      <div class="result-item">
        <span>Ahorro mensual necesario:</span>
        <span class="result-value">${fmt(mensual)}</span>
      </div>
      <div class="result-item">
        <span>Progreso actual:</span>
        <span class="result-value">${porcentajeCompleto.toFixed(1)}%</span>
      </div>
      <div class="result-item">
        <span>Tiempo restante:</span>
        <span class="result-value">${mesesRestantes} meses</span>
      </div>
    `;
  }
  ...
  simularCrecimiento(){
    const mensual = parseFloat(document.getElementById("ahorroMensual").value) || 0;
    const meses = parseInt(document.getElementById("periodoSimulador").value) || 12;
    ...
    for(let i = 1; i <= meses; i++){
      ...
      if(i % 6 === 0 || i === meses){
        resultados.push({mes: i, total: total, sinInteres: sinInteres});
      }
    }

    const intesesGanados = total - sinInteres;
    const grafico = document.getElementById("graficoSimulacion");
    if(!grafico) return;
    
    grafico.innerHTML = `
      <div style="width:100%">
        <h4>📈 Proyección de Ahorro</h4>
        <div style="display:grid;gap:12px;margin:20px 0">
          ${resultados.map(r => `
            <div style="display:flex;justify-content:space-between;align-items:center;padding:8px;background:#fff;border-radius:8px">
              <span>Mes ${r.mes}</span>
              <span style="font-weight:700;color:#059669">${fmt(r.total)}</span>
            </div>
          `).join('')}
        </div>
        <div style="background:#e0f2fe;padding:16px;border-radius:8px;margin-top:16px">
          ...
          <div style="display:flex;justify-content:space-between;margin-bottom:8px">
            ...
            <strong>${fmt(sinInteres)}</strong>
          </div>
          <div style="display:flex;justify-content:space-between;color:#0288d1">
            <span>Intereses ganados:</span>
            <strong>${fmt(intesesGanados)}</strong>
          </div>
        </div>
      </div>
    `;
  ...
  // Renderizar consejos personalizados
  renderConsejosAhorro(){
    const el = document.getElementById("consejosAhorro");
    if(!el) return;
    ...
    const totalGas = d.gastosFijos.reduce((s,x)=>s+(x.monto||0),0) + 
                     ...
                     d.creditos.reduce((s,x)=>s+(x.cuotaMensual||0),0) + 
                     d.gastosCompras.reduce((s,x)=>s+(x.monto||0),0);
    const libre = totalIng - totalGas;
    const porcentajeAhorro = totalIng > 0 ? (libre / totalIng) * 100 : 0;
    
    const consejos = [];
    
    // Consejos basados en el porcentaje de ahorro
    if(porcentajeAhorro < 10){
      consejos.push({
        titulo: "🚨 Mejora tu capacidad de ahorro",
        texto: "Estás ahorrando menos del 10% de tus ingresos. Trata de reducir gastos no esenciales y aumenta gradualmente tu tasa de ahorro hasta llegar al 20%."
      });
    } else if(porcentajeAhorro >= 20){
      consejos.push({
        titulo: "🏆 ¡Excelente disciplina de ahorro!",
        texto: "Estás ahorrando más del 20% de tus ingresos. Considera diversificar tus ahorros en diferentes tipos de inversiones para hacer crecer tu dinero."
      });
    }
    
    // Consejos sobre deudas
    const totalDeudas = d.tarjetas.reduce((s,x)=>s+(x.montoTotal||0),0) + 
                       d.creditos.reduce((s,x)=>s+(x.montoTotal||0),0);
    if(totalDeudas > totalIng * 3){
      consejos.push({
        titulo: "⚠️ Controla tus deudas",
        texto: "Tu nivel de deudas es alto comparado con tus ingresos. Prioriza pagar las deudas con mayor interés y evita adquirir nuevas deudas."
      });
    }
    ...
    consejos.push({
      ...
      texto: "Configura transferencias automáticas hacia una cuenta de ahorros el mismo día que recibes tu salario. Si no lo ves, no lo gastas."
    });
    
    consejos.push({
      titulo: "📱 Usa la regla de las 24 horas",
      texto: "Antes de hacer compras no planificadas mayores a $50,000, espera 24 horas. Muchas veces el impulso de compra desaparece."
    });
    
    consejos.push({
      titulo: "🎯 Define metas claras",
      texto: "Es más fácil ahorrar cuando tienes un objetivo específico: vacaciones, fondo de emergencia, o una compra importante."
    });
      ...
      <div class="consejos-container">
        ${consejos.map(c => `
          <div class="consejo-item">
            <div class="consejo-title">${c.titulo}</div>
            <div class="consejo-text">${c.texto}</div>
          </div>
        `).join('')}
      </div>
    `;
    ...
    this.consejoIndex = 0;
    setTimeout(() => this.actualizarConsejoVisible(), 100);
  }

  // Renderizar logros de ahorro
  renderLogrosAhorro(){
    const el = document.getElementById("logrosAhorro");
    if(!el) return;
    
    const d = this.mesData;
    const totalAhorrado = d.ahorros.reduce((s,x)=>s+(x.actual||0),0);
    const metasCompletadas = d.ahorros.filter(a => a.actual >= a.meta).length;
    const totalIng = d.ingresos.reduce((s,x)=>s+(x.monto||0),0);
    const totalGas = d.gastosFijos.reduce((s,x)=>s+(x.monto||0),0) + 
                     d.tarjetas.reduce((s,x)=>s+(x.cuotaMensual||0),0) + 
                     d.creditos.reduce((s,x)=>s+(x.cuotaMensual||0),0) + 
                     d.gastosCompras.reduce((s,x)=>s+(x.monto||0),0);
    const porcentajeAhorro = totalIng > 0 ? ((totalIng - totalGas) / totalIng) * 100 : 0;
    
    const logros = [
      {
        icon: "🌱",
        titulo: "Primer Ahorro",
        descripcion: "Guarda tu primer peso",
        desbloqueado: totalAhorrado > 0
      },
      {
        icon: "💰",
        titulo: "Ahorrador Principiante",
        descripcion: "Ahorra $100,000",
        desbloqueado: totalAhorrado >= 100000
      },
      {
        icon: "🏦",
        titulo: "Ahorrador Intermedio", 
        descripcion: "Ahorra $500,000",
        desbloqueado: totalAhorrado >= 500000
      },
      {
        icon: "💎",
        titulo: "Ahorrador Experto",
        descripcion: "Ahorra $1,000,000",
        desbloqueado: totalAhorrado >= 1000000
      },
      {
        icon: "🎯",
        titulo: "Cumplidor de Metas",
        descripcion: "Completa tu primera meta",
        desbloqueado: metasCompletadas >= 1
      },
      {
        icon: "📊",
        titulo: "Disciplina 20%",
        descripcion: "Ahorra más del 20% de tus ingresos",
        desbloqueado: porcentajeAhorro >= 20
      }
    ];
    
    el.innerHTML = logros.map(logro => `
      <div class="achievement-badge ${logro.desbloqueado ? '' : 'locked'}">
        <div class="achievement-icon">${logro.desbloqueado ? logro.icon : '🔒'}</div>
        <div class="achievement-title">${logro.titulo}</div>
        <div class="achievement-desc">${logro.descripcion}</div>
      </div>
    `).join('');
  }

  // Renderizar resumen de progreso para logros
  renderResumenProgreso(){
    const el = document.getElementById("resumenProgreso");
    if(!el) return;
    
    const d = this.mesData;
    const totalAhorrado = d.ahorros.reduce((s,x)=>s+(x.actual||0),0);
    const metasCompletadas = d.ahorros.filter(a => a.actual >= a.meta).length;
    const totalMetas = d.ahorros.length;
    
    // Calcular logros desbloqueados
    const logrosDesbloqueados = this.calcularLogrosDesbloqueados();
    const totalLogros = 6; // Número total de logros disponibles
    
    el.innerHTML = `
      <h4>🎯 Tu Progreso de Ahorro</h4>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:20px;margin-top:16px">
        <div style="text-align:center">
          <div style="font-size:32px;font-weight:800;color:#fbbf24">${fmt(totalAhorrado)}</div>
          <div style="font-size:14px;opacity:0.9">Total Ahorrado</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:32px;font-weight:800;color:#fbbf24">${metasCompletadas}/${totalMetas}</div>
          <div style="font-size:14px;opacity:0.9">Metas Completadas</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:32px;font-weight:800;color:#fbbf24">${logrosDesbloqueados}/${totalLogros}</div>
          <div style="font-size:14px;opacity:0.9">Logros Desbloqueados</div>
        </div>
      </div>
    `;
  }
  ...
  calcularLogrosDesbloqueados(){
    ...
    let desbloqueados = 0;
    if(totalAhorrado > 0) desbloqueados++;
    if(totalAhorrado >= 100000) desbloqueados++;
    if(totalAhorrado >= 500000) desbloqueados++;
    if(totalAhorrado >= 1000000) desbloqueados++;
    if(metasCompletadas >= 1) desbloqueados++;
    if(porcentajeAhorro >= 20) desbloqueados++;
    
    return desbloqueados;
  }

  // Renderizar consejo destacado
  renderConsejoDestacado(){
    const el = document.getElementById("consejoDestacado");
    if(!el) return;
    ...
    if(porcentajeAhorro < 5){
      consejo = {
        titulo: "🚨 Prioridad: Crear Capacidad de Ahorro",
        texto: "Tu capacidad de ahorro es muy baja. Comienza identificando gastos no esenciales que puedas reducir gradualmente."
      };
    } else if(porcentajeAhorro < 15){
      consejo = {
        titulo: "📈 Aumenta tu Tasa de Ahorro",
        texto: "¡Buen comienzo! Trata de incrementar tu ahorro al 15-20% aplicando la regla 50-30-20."
      };
    } else {
      consejo = {
        titulo: "🏆 ¡Excelente Disciplina!",
        texto: "Tienes una gran capacidad de ahorro. Considera diversificar en inversiones para hacer crecer tu dinero."
      };
    }
    
    el.innerHTML = `
      <h4>${consejo.titulo}</h4>
      <p style="margin:12px 0 0 0;font-size:16px;opacity:0.95">${consejo.texto}</p>
    `;
  }
  ...
  anteriorConsejo(){
    this.consejoIndex = Math.max(0, this.consejoIndex - 1);
    this.actualizarConsejoVisible();
  }
  
  siguienteConsejo(){
    const totalConsejos = document.querySelectorAll('#consejosAhorro .consejo-item').length;
    this.consejoIndex = Math.min(totalConsejos - 1, this.consejoIndex + 1);
    this.actualizarConsejoVisible();
  }
  
  actualizarConsejoVisible(){
    const consejos = document.querySelectorAll('#consejosAhorro .consejo-item');
    consejos.forEach((consejo, index) => {
      consejo.style.display = index === this.consejoIndex ? 'block' : 'none';
    });
  }
  ...
  analisisRapido(){
    ...
    const resultado = document.getElementById("analisisRapidoResultado");
    if(!resultado) return;
    
    let status = "🔴 Crítico";
    let color = "#ef4444";
    if(porcentajeAhorro >= 5) { status = "🟡 Mejorable"; color = "#f59e0b"; }
    if(porcentajeAhorro >= 15) { status = "🟢 Bueno"; color = "#10b981"; }
    if(porcentajeAhorro >= 20) { status = "🏆 Excelente"; color = "#8b5cf6"; }
    
    resultado.innerHTML = `
      <div style="background:${color}15;border:1px solid ${color}40;padding:16px;border-radius:12px;margin-top:16px">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div>
            <div style="font-weight:600;color:${color}">${status}</div>
            <div style="font-size:14px;color:#6b7280;margin-top:4px">Capacidad de ahorro: ${porcentajeAhorro.toFixed(1)}%</div>
          </div>
          <div style="font-size:24px;font-weight:800;color:${color}">${fmt(libre)}</div>
        </div>
      </div>
    `;
  }
  ...
  calcularEmergencia(){
    ...
    resultado.innerHTML = `
      <div style="background:#e0f2fe;border:1px solid #0284c7;padding:16px;border-radius:12px;margin-top:16px">
        <h4 style="margin:0 0 12px 0;color:#0369a1">🚨 Fondo de Emergencia</h4>
        <div style="display:flex;justify-content:space-between;margin-bottom:8px">
          <span>Mínimo (3 meses):</span>
          <strong style="color:#0369a1">${fmt(emergencia3)}</strong>
        </div>
        <div style="display:flex;justify-content:space-between">
          <span>Ideal (6 meses):</span>
          <strong style="color:#0369a1">${fmt(emergencia6)}</strong>
        </div>
      </div>
    `;
  }
  ...
  optimizarGastos(){
    const d = this.mesData;
    const totalIng = d.ingresos.reduce((s,x)=>s+(x.monto||0),0);
    const gastosFijos = d.gastosFijos.reduce((s,x)=>s+(x.monto||0),0);
    const gastosCompras = d.gastosCompras.reduce((s,x)=>s+(x.monto||0),0);
    ...
    if(gastosFijos > totalIng * 0.6){
      optimizaciones.push("⚠️ Gastos fijos muy altos (>60% ingresos)");
    }
    if(gastosCompras > totalIng * 0.25){
      optimizaciones.push("🛒 Gastos variables elevados (>25% ingresos)");
    }
    if(optimizaciones.length === 0){
      optimizaciones.push("✅ Distribución de gastos saludable");
    }
    
    resultado.innerHTML = `
      <div style="background:#f0fdf4;border:1px solid #22c55e;padding:16px;border-radius:12px;margin-top:16px">
        <h4 style="margin:0 0 12px 0;color:#166534">💡 Optimización de Gastos</h4>
        ${optimizaciones.map(opt => `<div style="margin:4px 0;color:#15803d">${opt}</div>`).join('')}
      </div>
    `;
  }
  ...
  escenarioConservador(){
    document.getElementById("ahorroMensual").value = "150000";
    document.getElementById("periodoSimulador").value = "36";
    document.getElementById("tasaInteres").value = "3";
    this.simularCrecimiento();
  }

  escenarioModerado(){
    document.getElementById("ahorroMensual").value = "200000";
    document.getElementById("periodoSimulador").value = "24";
    document.getElementById("tasaInteres").value = "7";
    this.simularCrecimiento();
  }

  escenarioAgresivo(){
    document.getElementById("ahorroMensual").value = "300000";
    document.getElementById("periodoSimulador").value = "18";
    document.getElementById("tasaInteres").value = "12";
    this.simularCrecimiento();
  }
}
